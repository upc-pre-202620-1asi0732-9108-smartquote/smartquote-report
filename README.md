...
Las nuevas historias mantienen los identificadores existentes de las épicas EP04, EP05 y EP06 para ampliar las capacidades ya definidas. Se incorpora EP02 para autenticación, que no estaba registrada previamente; US15 se asigna a EP07 para la gestión de proveedores y US16 a EP08 para mantener la analítica del proceso de compras como un objetivo diferenciado.
...
| **Story ID** | **User** | **Priority** | **Epic** |
| --- | --- | --- | --- |
| US09 | Usuario del sistema (analista, especialista o jefe de compras) | Alta | EP02 – Autenticación y seguridad de acceso |
| **Title** | Registrarse e iniciar sesión de forma segura en la plataforma |  |  |
| **Description** | Como usuario del sistema de adquisiciones avícolas, deseo registrarme e iniciar sesión con credenciales seguras para acceder a los módulos de compras asignados a mi rol. |  |  |
| **Acceptance Criteria** | **Escenario 1: Registro con credenciales válidas**<br>**Dado que** una persona ingresa datos de registro válidos y una contraseña que cumple la política de seguridad<br>**Cuando** solicita crear su cuenta<br>**Entonces** el sistema registra la cuenta, protege la contraseña y confirma que el usuario puede iniciar sesión según los permisos asociados a su rol.<br><br>**Escenario 2: Contraseña insegura o correo ya registrado**<br>**Dado que** los datos de registro incumplen la política de contraseña o el correo ya está asociado a una cuenta<br>**Cuando** la persona intenta registrarse<br>**Entonces** el sistema rechaza la solicitud, identifica el campo que debe corregirse y no crea una cuenta duplicada.<br><br>**Escenario 3: Inicio de sesión**<br>**Dado que** el usuario posee una cuenta activa<br>**Cuando** envía credenciales correctas o incorrectas<br>**Entonces** el sistema permite el acceso solo con credenciales válidas y presenta un mensaje claro sin revelar si una contraseña específica es correcta. |  |  |

| **Story ID** | **User** | **Priority** | **Epic** |
| --- | --- | --- | --- |
| TS02 | Developer | Alta | EP02 – Autenticación y seguridad de acceso |
| **Title** | Implementar autenticación y control de acceso basado en roles mediante JWT |  |  |
| **Description** | Como Developer, deseo implementar un servicio de autenticación mediante tokens JWT y control de acceso basado en roles (RBAC) para asegurar que cada endpoint de la API RESTful valide la identidad y los permisos de la solicitud. |  |  |
| **Acceptance Criteria** | **Escenario 1: Emisión de un token autenticado**<br>**Dado que** un usuario presenta credenciales válidas<br>**Cuando** la API autentica al usuario<br>**Entonces** emite un token firmado con identidad, rol y vigencia, sin incluir contraseñas ni secretos.<br><br>**Escenario 2: Acceso a un endpoint protegido**<br>**Dado que** un cliente solicita un endpoint protegido con un token ausente, vencido o inválido<br>**Cuando** la API autoriza la solicitud<br>**Entonces** rechaza el acceso con una respuesta HTTP 401 y no ejecuta la operación.<br><br>**Escenario 3: Permisos insuficientes**<br>**Dado que** un usuario autenticado carece del rol requerido para una acción<br>**Cuando** intenta ejecutar una operación restringida, como aprobar una orden de compra<br>**Entonces** la API responde HTTP 403 y conserva sin cambios la información protegida. |  |  |

| **Story ID** | **User** | **Priority** | **Epic** |
| --- | --- | --- | --- |
| US10 | Analista de adquisiciones | Alta | EP04 – Gestión inteligente de cotizaciones |
| **Title** | Incorporar cotizaciones mediante carga masiva de archivos PDF |  |  |
| **Description** | Como analista de adquisiciones, deseo subir múltiples archivos PDF de cotizaciones simultáneamente en un solo lote para agilizar el procesamiento de ofertas recibidas de diversos proveedores avícolas. |  |  |
| **Acceptance Criteria** | **Escenario 1: Carga de un lote válido**<br>**Dado que** existe una solicitud activa y el analista selecciona varios archivos PDF permitidos<br>**Cuando** confirma la carga del lote<br>**Entonces** el sistema registra cada documento por separado, lo vincula con la solicitud y el proveedor indicado, y comunica el estado de procesamiento de cada archivo.<br><br>**Escenario 2: Lote con archivos inválidos**<br>**Dado que** el lote contiene archivos válidos y archivos dañados, duplicados o con formato no permitido<br>**Cuando** el sistema valida el lote<br>**Entonces** incorpora los documentos válidos, rechaza los no procesables e informa el resultado y la causa para cada archivo.<br><br>**Escenario 3: Consulta del resultado del lote**<br>**Dado que** el procesamiento de uno o más documentos sigue en curso o ha finalizado<br>**Cuando** el analista consulta el lote<br>**Entonces** el sistema presenta por archivo el estado, el resultado disponible y cualquier acción requerida, sin bloquear la consulta de los demás documentos. |  |  |

| **Story ID** | **User** | **Priority** | **Epic** |
| --- | --- | --- | --- |
| SP02 | Developer | Alta | EP04 – Gestión inteligente de cotizaciones |
| **Title** | Evaluar la viabilidad del procesamiento asíncrono de lotes de documentos PDF |  |  |
| **Description** | Como Developer, deseo investigar y realizar una prueba de concepto sobre colas de procesamiento asíncrono y trabajadores en segundo plano para determinar el rendimiento y los límites de consumo de recursos al procesar lotes grandes de PDF sin congelar la aplicación. |  |  |
| **Acceptance Criteria** | **Escenario 1: Prueba de procesamiento por lotes**<br>**Dado que** se dispone de un conjunto representativo de cotizaciones PDF anonimizadas<br>**Cuando** se ejecuta una prueba de concepto con procesamiento síncrono y asíncrono<br>**Entonces** se registran el tiempo de respuesta, la duración total, el uso de recursos, la concurrencia y la tasa de fallos de cada alternativa.<br><br>**Escenario 2: Manejo de fallos y reintentos**<br>**Dado que** un documento falla durante el procesamiento o el trabajador se interrumpe<br>**Cuando** se ejecuta la prueba de concepto<br>**Entonces** se determina si el trabajo puede reintentarse de forma controlada sin duplicar resultados ni detener el procesamiento de los demás documentos.<br><br>**Escenario 3: Cierre del spike**<br>**Dado que** se han obtenido resultados bajo las cargas de prueba definidas<br>**Cuando** concluye la investigación<br>**Entonces** se entrega una recomendación documentada sobre arquitectura, límites de lote, estrategia de reintentos, monitoreo, costos y criterios para adoptar o descartar la alternativa. |  |  |

| **Story ID** | **User** | **Priority** | **Epic** |
| --- | --- | --- | --- |
| TS03 | Developer | Media | EP05 – Evaluación y decisión de compra |
| **Title** | Integrar una fuente de tipo de cambio para la conversión de cotizaciones |  |  |
| **Description** | Como Developer, deseo consumir un servicio web oficial de tipo de cambio, como los publicados por SUNAT o SBS, para convertir automáticamente las cotizaciones en dólares estadounidenses (USD) a soles peruanos (PEN) durante la simulación de precios. |  |  |
| **Acceptance Criteria** | **Escenario 1: Conversión con una cotización disponible**<br>**Dado que** una cotización se encuentra expresada en USD y la fuente oficial configurada publica el tipo de cambio aplicable<br>**Cuando** se ejecuta la simulación de precios<br>**Entonces** el sistema convierte los importes a PEN usando la tasa publicada y conserva la moneda y el importe originales.<br><br>**Escenario 2: Trazabilidad de la tasa utilizada**<br>**Dado que** una simulación emplea una tasa de cambio<br>**Cuando** se consulta el resultado<br>**Entonces** el sistema informa la tasa, la moneda de origen y destino, la fuente oficial y la fecha y hora de consulta para permitir reproducir el cálculo.<br><br>**Escenario 3: Fuente no disponible**<br>**Dado que** la fuente oficial de tipo de cambio no responde o no entrega una tasa vigente<br>**Cuando** se intenta realizar la conversión<br>**Entonces** el sistema comunica que la conversión no pudo completarse y no sustituye la tasa por un valor desactualizado sin autorización explícita. |  |  |
...
| **Story ID** | **User** | **Priority** | **Epic** |
| --- | --- | --- | --- |
| TS04 | Developer | Media | EP06 – Orden y trazabilidad |
| **Title** | Exponer un endpoint RESTful para integrar órdenes de compra con sistemas externos |  |  |
| **Description** | Como Developer, deseo exponer un endpoint RESTful que entregue el detalle de las órdenes de compra en JSON normalizado para permitir su sincronización con los sistemas de inventario o contabilidad de la empresa. |  |  |
| **Acceptance Criteria** | **Escenario 1: Consulta de una orden autorizada**<br>**Dado que** un sistema consumidor autenticado solicita una orden existente<br>**Cuando** consulta el endpoint de integración<br>**Entonces** la API responde con JSON conforme al contrato publicado, incluyendo identificador, proveedor, partidas, cantidades, precios, moneda, estado y fechas relevantes.<br><br>**Escenario 2: Consulta de órdenes inexistentes o no autorizadas**<br>**Dado que** el consumidor solicita un identificador inexistente o carece de permisos<br>**Cuando** la API procesa la solicitud<br>**Entonces** responde con el código HTTP correspondiente y un mensaje estructurado sin exponer información de otras órdenes.<br><br>**Escenario 3: Contrato de integración**<br>**Dado que** el sistema consumidor integra el endpoint<br>**Cuando** procesa una respuesta válida<br>**Entonces** los nombres, tipos y significados de los campos coinciden con la documentación OpenAPI y los errores mantienen un formato consistente. |  |  |
